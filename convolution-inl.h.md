...
namespace conv {//声明Convolution的输入变量
enum ConvolutionOpInputs {kData, kWeight, kBias};
enum ConvolutionOpOutputs {kOut};
enum ConvolutionOpResource {kTempSpace};
}

struct ConvolutionParam : public dmlc::Parameter<ConvolutionParam> {//卷积层的参数，核大小，步长等内容
  TShape kernel;
  TShape stride;
  TShape pad;
  uint32_t num_filter;
  ...
  uint64_t workspace;
  bool no_bias;
  DMLC_DECLARE_PARAMETER(ConvolutionParam) {//对前面参数的解释
    int shape[] = {1, 1};
...
class ConvolutionOp : public Operator {//操作类，主要是前向和反向如何执行Forward和Backward函数
 public:
  ...
  virtual void Forward(const OpContext &ctx,
                       ...
                       const std::vector<TBlob> &aux_args) {//Forword函数开始
    using namespace mshadow;
    using namespace mshadow::expr;
    CHECK_EQ(req[conv::kOut], kWriteTo);
    size_t expected = param_.no_bias ? 2 : 3;
    CHECK_EQ(in_data.size(), expected);
    CHECK_EQ(out_data.size(), 1);//判断输入参数的大小是否正确
...
#endif
    const index_t nbatch = data.size(0);//将输入输入参数保存下来，输入保存为data，parameter保存为wmat，输出保存为out
    ...
    for (index_t i = 0; i < nbatch; i += nstep_) {
      ...
      Tensor<xpu, 3> temp_dst = Tensor<xpu, 3>(workspace.dptr_ + temp_col.shape_.Size(),
                                               Shape3(shape_dstunit_[0],
                                                      shape_dstunit_[1],
                                                      shape_dstunit_[2] * step), s);//定义了两个中间变量,temp_col，temp_dst
